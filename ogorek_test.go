...
// TestPickle represents a test pickle that pickle encoder produces at particular protocols.
//
// If protov is empty there is no connection in between pickle encoder and the
// data. However the test data can still be used to feed pickle decoder.
...
var tests = []TestEntry{
	...
	// the only case where pickle encoding is loosy is for Go struct types.
	Xloosy("[]pickle.foo{\"Qux\", 4}", []foo{{"Qux", 4}},
...
// TestDecode verifies pickle decoder.
...
// TestEncode verifies pickle encoder.
